 ...
 * 
 * One’s Compliment Operator aka bitwise complement
 * inverts each bit of operand
...
struct area OR;			// |
struct area XOR;		// ^
struct area complement;		// ~
struct area shift_left;		// >>
struct area shift_right;	// <<
...
temp_surface=TTF_RenderText_Shaded(font,"~",font_color, bgcolor);
complement.Texture= SDL_CreateTextureFromSurface(Renderer, temp_surface);
SDL_QueryTexture(complement.Texture, NULL, NULL, &complement.dst.w, &complement.dst.h);
complement.dst.y=20;
complement.dst.x=WW-50-complement.dst.w;
//END 	Shaded
rect_copy(&complement);
//END 	COMPLEMENT
//LE
temp_surface = IMG_Load( "./assets/l_e.png" );

le.Texture = SDL_CreateTextureFromSurface( Renderer, temp_surface );
...
//BE
temp_surface = IMG_Load( "./assets/b_e.png" );
be.Texture = SDL_CreateTextureFromSurface( Renderer, temp_surface );
...
//NS
temp_surface=TTF_RenderText_Blended(font,"2", font_color);
ns.Texture = SDL_CreateTextureFromSurface( Renderer, temp_surface );
...
SDL_SetWindowTitle(Window, "C Bit Ops");
	...
	while(SDL_PollEvent(&event)){
		...
		if(event.type == SDL_MOUSEBUTTONDOWN){
			...
			if(event.button.button == SDL_BUTTON_LEFT){
				if (SDL_PointInRect(&mouse, &complement.dst)){
					line=~line;
					check_line();
					render_text();
				}
...
for (i=0; i <8; i++){
	...
	SDL_RenderCopy(Renderer, complement.Texture, NULL, &complement.dst);
...
void render_text(void)
{
	...
	temp_surface=TTF_RenderText_Blended(font,buffer, font_color);
	byte_numbers.Texture = SDL_CreateTextureFromSurface( Renderer, temp_surface );
	int w,h;
	SDL_QueryTexture(byte_numbers.Texture, NULL, NULL, &w, &h);
	if (WW!=RES_W){
